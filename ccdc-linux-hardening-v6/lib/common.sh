...
# Dependency checking functions - silently skip if not available
...
function require_service {
    local service_name="$1"
    local friendly_name="${2:-$service_name}"
    
    if ! check_service_installed "$service_name"; then
        log_info "$friendly_name is not installed - skipping this configuration"
        return 1
    fi
    return 0
}
...
function require_directory {
    local dir_path="$1"
    local friendly_name="${2:-$dir_path}"
    
    if ! check_directory_exists "$dir_path"; then
        log_info "$friendly_name directory not found - skipping this configuration"
        return 1
    fi
    return 0
}
...
function require_file {
    local file_path="$1"
    local friendly_name="${2:-$file_path}"
    
    if ! check_file_exists "$file_path"; then
        log_info "$friendly_name file not found - skipping this configuration"
        return 1
    fi
    return 0
}
...
function skip_if_missing {
    local check_type="$1"
    local check_value="$2"
    local friendly_name="${3:-$check_value}"
    
    case "$check_type" in
        service)
            require_service "$check_value" "$friendly_name" || return 1
            ;;
        directory)
            require_directory "$check_value" "$friendly_name" || return 1
            ;;
        file)
            require_file "$check_value" "$friendly_name" || return 1
            ;;
    esac
    return 0
}
